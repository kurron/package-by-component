...
class MeleeWeaponRepositoryTest {
    ...
    fun verifyCRUD() {
        ...
        val toSave = MeleeWeapon(technologyLevel = 1, weapon = "Axe", damage = "swinging + 2", damageType = "cutting", cost = 50, weight = 4, strength = 11)
        val written = sut.save(toSave)
        val read = sut.findById(written.id)
        val technologyLevel = ThreadLocalRandom.current().nextInt(Int.MAX_VALUE)
        val toUpdate = read.get().copy(technologyLevel = technologyLevel)
        Thread.sleep(Duration.ofSeconds(2))
        sut.save(toUpdate)
        val all = sut.findAll()
        assertEquals(1, all.size, "Unexpected result size!")
        assertEquals(technologyLevel, all.first().technologyLevel, "Tech levels do no match!")
